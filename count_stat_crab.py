...
edge = '/uufs/astro.utah.edu/common/home/u1019304/temp/fullmask'+detp+'_final.fits'
...
orig_data = homedir+'event_cl/nu'+obsid+detp+'01_cl.evt'
# Load data
...
#  global A_bins,det0_bins,det1_bins,det2_bins,det3_bins 
  mod = (x_c*A_bins) + (y*det0_bins) + (z*det1_bins) + (w*det2_bins) + (t*det3_bins) + (u*Cn_bins)
...
def lmmin(pars,data=None):
  mod = x_c*A_bins + pars['det0'].value*det0_bins + pars['det1'].value*det1_bins + pars['det2'].value*det2_bins + pars['det3'].value*det3_bins + pars['crab'].value*Cn_bins
  if data is None:
    return mod
  return mod - data
#############################################################   
...
#fit_params.add('aCXB', value=x_c, min=0.0)
fit_params.add('det0', value=det0_initval, min=(det0_initval*0.0001))
