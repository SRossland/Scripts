...
# syntax: ./mkimgsscreen.py dir obsid subdir param elowlist ehighlist
...
try:
    ...
    obsids=[sys.argv[2]]
    dir=sys.argv[1]+"/"+obsids[0]+"/event_cl/"+sys.argv[3]+'/'
else:
    obsids = fobs.readlines()
    for i in range(len(obsids)):
        obsids[i]=obsids[i].rstrip()
    fobs.close()
    dir=sys.argv[1]+'/'+sys.argv[2]+'/'+sys.argv[3]+'/'

slow=sys.argv[5].split(',')
shigh=sys.argv[6].split(',')
...
for det in ["A","B"]:
    ...
    for obsid in obsids:
        ...
        xsel.write("read event "+edir+"/event_cl/"+sys.argv[3]+"/nu"+obsid+det+"01_fullevts_"+sys.argv[4]+".fits\n")
        if iobs == 0:
            xsel.write("./\n")
            xsel.write("yes\n")
        iobs=iobs+1
    ...
    for i in range(len(slow)):
        ...
        xsel.write(dir+"im"+det+slow[i]+"to"+shigh[i]+sys.argv[4]+"keV.fits\n")
        xsel.write("clear\n")
