...
det3_bins = np.asarray(det3_bins)*0  # Here
...
######################
def fn(params, z=det1_initval, w=det2_initval, t=0):#t=det3_initval):
#  x,y,z,w,t = params
  x = params['aCXB']
  y = params['det0']
#  z = params['det1']
...
#  global A_bins,det0_bins,det1_bins,det2_bins,det3_bins 
  mod = (x*A_bins) + (y*det0_bins) + (y*det1_rat*det1_bins) + (y*det2_rat*det2_bins) #+ (y*det3_rat*det3_bins)
...
def fn_cerror(params, z=det1_initval, w=det2_initval,t=0): #t=det3_initval):
  y = params['det0']
#  z = params['det1']
#  w = params['det2']
#  t = params['det3']
  mod = (x_c*A_bins) + (y*det0_bins) + (y*det1_rat*det1_bins) + (y*det2_rat*det2_bins) #+ (y*det3_rat*det3_bins)
  numerA = mod - count_bins + count_bins*(np.log(count_bins)-np.log(mod))
  return (2 * np.sum(numerA))
####################################
...
def lmmin(pars,data=None, z=det1_initval, w=det2_initval, t=det3_initval):
  mod = pars['aCXB']*A_bins + pars['det0']*det0_bins + pars['det0']*z*det1_bins + pars['det0']*w*det2_bins + pars['det0']*t*det3_bins
  if data is None:
    return mod
  return mod - data
...
def getDetInit(tele,detector,lener,hener):
    values = np.genfromtxt('/uufs/astro.utah.edu/common/home/u1019304/detvals/{}det{}normvals.txt'.format(tele,detector))
    idx_low = (np.abs(values[:,0]-float(lener))).argmin()
    idx_high = (np.abs(values[:,1]-float(hener))).argmin()
    #idx_low = np.where(values[:,0]==float(lener))[0]
    #idx_high = np.where(values[:,1]==float(hener))[0]
    if idx_low != idx_high:
        if (np.abs(values[idx_low,0]-float(lener)) < np.abs(values[idx_high,1]-float(hener))):
            idx = copy.copy(idx_low)
        else:
            idx = copy.copy(idx_high)
...
det0_initval = getDetInit(detp, 0, lowe, highe)
det1_initval = getDetInit(detp, 1, lowe, highe)
det2_initval = getDetInit(detp, 2, lowe, highe)
det3_initval = getDetInit(detp, 3, lowe, highe)
...
aCXB_initval *= Exposure; aCXB_initval *= (float(highe)-0.04-float(lowe))
det0_initval *= Exposure; det0_initval *= (float(highe)-0.04-float(lowe))
det1_initval *= Exposure; det1_initval *= (float(highe)-0.04-float(lowe))
det2_initval *= Exposure; det2_initval *= (float(highe)-0.04-float(lowe))
det3_initval *= Exposure; det3_initval *= (float(highe)-0.04-float(lowe))

#det1_initval /= det0_initval
#det2_initval /= det0_initval
#det3_initval /= det0_initval

det1_rat = det1_initval/det0_initval
det2_rat = det2_initval/det0_initval
det3_rat = det3_initval/det0_initval
...
fit_params.add('det0', value=det0_initval, max=det0_initval)
#fit_params.add('det1', value=det1_initval, min=det1_initval)
#fit_params.add('det2', value=det2_initval, min=det2_initval)
#fit_params.add('det3', value=det3_initval, min=det3_initval)
...
def fitcstat(x,y,z=det1_initval, w=det2_initval, t=det3_initval):
#  global A_bins,det0_bins,det1_bins,det2_bins,det3_bins 
  mod = (x*A_bins) + (y*det0_bins) + (y*det1_rat*det1_bins) + (y*det2_rat*det2_bins) #+ (y*det3_rat*det3_bins)
  numerA = mod - count_bins + count_bins*(np.log(count_bins)-np.log(mod))
  return (2 * np.sum(numerA))
...
#        stand['det3'].set(value=newdetvals[3])
  ...
  with open(homedir+'/'+detp+"_high_params_errors.txt",'a+') as O:
    O.write(str(x_c)+' '+str(fit_params_errors['det0'].value)+' '+str(fit_params_errors['det0'].value*det1_rat)+' '+str(fit_params_errors['det0'].value*det2_rat)+' '+str(fit_params_errors['det0'].value*det3_rat)+'\n')
#  fit_params_errors=None

if len(newdetvals) > 0:
	resetParams(newdetvals[0])
	x_c = None
	x_c = newaCXB
	...
	fit_params_errors = None
	fit_params_errors = Parameters()
	fit_params_errors.add('det0', value=stand['det0'].value, min=0.0)
#	fit_params_errors.add('det1', value=fit_params['det1'].value, min=0.0)
#	fit_params_errors.add('det2', value=fit_params['det2'].value, min=0.0)
#	fit_params_errors.add('det3', value=fit_params['det3'].value, min=0.0)
	newcstat=np.copy(cstat)
...
#  fit_params_errors.add('det3', value=fit_params['det3'].value, min=0.0)
  resA = minimize(fn_cerror, fit_params_errors, method = 'nelder', tol=1e-15)
  newcstat = fitcstat(x_c,stand['det0'].value) #,fit_params_errors['det1'].value,fit_params_errors['det2'].value,fit_params_errors['det3'].value)
...
with open(homedir+'/'+detp+"_params.txt",'a+') as O:
  O.write(lowe+' '+highe+' '+str(stand['aCXB'].value)+' '+str(sigup)+' '+str(sigdown)+' '+str(Exposure)+'\n')
...
with open(homedir+'/'+detp+'_'+sep+'_'+lowe+'_'+highe+'_'+'keVparams_longformat.txt','a+') as O:
    ...
    O.write("det1 norm value: "+str(fit_params['det0'].value*det1_rat)+'\n')
    O.write("det2 norm value: "+str(fit_params['det0'].value*det2_rat)+'\n')
    #O.write("det3 norm value: "+str(fit_params['det0'].value*det3_initval)+'\n')
    O.write("#"*25+'\n')
...
model_data = (stand['aCXB'].value*Aij+stand['det0'].value*a0+stand['det0'].value*a1*det1_rat+stand['det0'].value*a2*det2_rat+stand['det0'].value*a3*det3_rat)*eA_M  # THIS NEEDS TO BE MULTI BY THE EXP IMAGE
model = (stand['aCXB'].value*Aij+stand['det0'].value*a0+stand['det0'].value*a1*det1_rat+stand['det0'].value*a2*det2_rat+stand['det0'].value*a3*det3_rat)
...
#  hdul[0].header['DET1NORM'] = str(stand['det0'].value*det1_rat)
#  hdul[0].header['DET2NORM'] = str(stand['det0'].value*det2_rat)
  #hdul[0].header['DET3NORM'] = str(stand['det0'].value*det3_rat)
#  hdul[0].header['COMMENT'] = 'Model for '+detp+' Energy: '+str(lowe)+' '+str(highe)
#  hdul.flush()
